...
class Beit3LlavaLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        if hasattr(config, "num_query"):
            ...
            self.query = torch.nn.Parameter(torch.zeros(
                config.num_query, self.vision_tower.args.encoder_embed_dim))
    ...
    def initialize_vision_modules(self, vision_tower, no_randaug, num_query):
        ...
        if not hasattr(self, 'vision_tower'):
            ...
            state_dict = torch.load(
                '/mnt/data/user/tc_agi/multi_modal/checkpoints/beit-v3/beit3_large_patch16_224.pth', map_location='cuda')['model']
        ...
        if (num_query is not None) and (not hasattr(self, 'query')):
            ...
            query_init_weight = [bos_weight] + \
                [None] * (num_query - 2) + [eos_weight]
        ...
        if vision_tower is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            if mask_query2img:
                ...
                uni_mask = torch.zeros(
                    (max_len, max_len), dtype=torch.long, device=input_ids.device)
            ...
            if type(images) is list:
                ...
                for image in images:
                    image_forward_out = vision_tower(pixel_values=image.unsqueeze(
                        0), query_embed=self.query, attn_mask=attn_mask)
                    image_features.append(image_forward_out)
            else:
                image_features = vision_tower(
                    pixel_values=images, query_embed=self.query, attn_mask=attn_mask)
            if type(images) is list:
            ...
            dummy_image_features = torch.zeros(
                self.config.num_query, vision_tower.hidden_size, device=inputs_embeds.device, dtype=inputs_embeds.dtype)
...
class Beit3LlavaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config, mm_vision_tower=None):
        ...
        self.model = Beit3LlavaLlamaModel(
