...
from omnilmm.eval.omnilmm_vqa import expand_question_into_multimodal, KeywordsStoppingCriteria
...
def init_omni_lmm(model_path, device=None, tune_clip=False):
...
def wrap_question_for_omni(question, tokenizer):
    conversation = [
        {
            'role': 'user',
            'content': question
...
class OmniLMMForSingleTurnChat:
    ...
    def llm_chat(self, question):
        input_ids = wrap_question_for_omni(
...
class OmniLMMForMultiTurnChat(OmniLMMForSingleTurnChat):
    def __init__(self, model, img_processor, image_token_len, tokenizer) -> None:
        super(OmniLMMForMultiTurnChat, self).__init__(
    ...
    def resume(self, question):
        ...
        input_ids = omni_preprocess(sources=[conv],
                                    tokenizer=tokenizer,
...
if __name__ == '__main__':
    ...
    model, img_processor, image_token_len, tokenizer = init_omni_lmm(
        '/home/yutianyu/Zephyr_checkpoints/SFT_exp/omni_lmm_12b_SFT-6node_5kPT_SFT_stage3-caterpillar-stage2_mix#caterpillar-stage3_lvis#caterpillar-stage3_svit#caterpillar-stage3_sharegpt4v#llava#unimm-chat-134#222#600#101#157#117/checkpionts/checkpoint-4000', tune_clip=tune_clip)
    chat_model = OmniLMMForSingleTurnChat(
        model, img_processor, image_token_len, tokenizer)
    chat_model = OmniLMMForMultiTurnChat(
        model, img_processor, image_token_len, tokenizer)
