...
from omnilmm.conversation import (default_conversation, conv_templates,
                                 SeparatorStyle)
from omnilmm.constants import LOGDIR
from omnilmm.utils import (build_logger, server_error_msg,
                          violates_moderation, moderation_msg)
from omnilmm.serve.gradio_patch import Chatbot as grChatbot
from omnilmm.serve.gradio_css import code_highlight_css
import hashlib
    ...
    with gr.Blocks(title="Muffin", theme=gr.themes.Base(), css=css) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=6):
                chatbot = grChatbot(
                    elem_id="chatbot", label=f"{model_name} Chatbot", visible=False).style(height=550)
                with gr.Row():
