...
from omnilmm.constants import WORKER_HEART_BEAT_INTERVAL
from omnilmm.utils import (build_logger, server_error_msg,
    ...
    if True:
        ...
        if vision_tower.beit3.text_embed.weight.device.type == 'meta':
            vision_tower = timm.create_model(
                model.model.config.mm_vision_tower)
            state_dict = torch.load(
                '/mnt/data/user/tc_agi/multi_modal/checkpoints/beit-v3/beit3_large_patch16_224.pth')['model']
    ...
    def generate_stream(self, params):
        ...
        if images is not None and self.is_multimodal:
            ...
            if len(images) > 0:
                ...
                images = [Image.open(BytesIO(base64.b64decode(image)))
                          for image in images]
                ...
                if self.keep_aspect_ratio:
                    ...
                    for image_idx, image in enumerate(images):
                        ...
                        prompt = prompt.replace(
                            DEFAULT_IMAGE_TOKEN, replace_token, 1)
                    images = new_images
                else:
                    images = torch.stack([image_processor(img)
                                         for img in images])
                    images = images.to(self.model.device, dtype=torch.float16)
                    # HACK: 64 is the max image token length hacked
                    replace_token = DEFAULT_IMAGE_PATCH_TOKEN * 64
                    if getattr(self.model.config, 'mm_use_im_start_end', False):
                        replace_token = DEFAULT_IM_START_TOKEN + replace_token + DEFAULT_IM_END_TOKEN
